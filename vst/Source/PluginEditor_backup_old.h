...
public:
        ...
        : AudioProcessorEditor(&p), proc(p)
    {
        ...
        auto setupKnob = [&](juce::Slider& s, const juce::String& paramId,
            ...
            s.setColour(juce::Slider::textBoxTextColourId, juce::Colour(0xff555555));
            ...
            lbl->setFont(juce::Font(9.0f, juce::Font::plain));
    ...
    void paint(juce::Graphics& g) override {
        ...
        g.drawText("0MGE", getWidth() / 2 - 45, 14, 90, 36, juce::Justification::centred);

        g.setColour(juce::Colour(0xffbbbbbb));
        g.setFont(juce::Font(9.0f, juce::Font::plain));
        g.drawText("granular engine", getWidth() / 2 - 50, 48, 100, 14, juce::Justification::centred);
        ...
        g.setColour(juce::Colour(0xffbbbbbb));
        g.setFont(juce::Font(9.0f, juce::Font::plain));
        g.drawText("by 0penAGI x TeMeT x Slut Online",
    ...
    void timerCallback() override {
        ...
        for (auto& pt : particles) {
            ...
            if (pt.life <= 0.0f || pt.x < -80 || pt.x > plugW + 80 ||
                ...
                float spawnY = vizTop + pt.homeY * vizH + ((float)(std::rand() % 1000) / 1000.0f - 0.5f) * 30.0f;
                pt.x = spawnX;
                pt.y = spawnY;
...
private:
    ...
    void drawParticles(juce::Graphics& g) {
        ...
        if (audioEnergy < 0.02f) return;

        for (auto& pt : particles) {
            float alpha = std::clamp(pt.life, 0.0f, 1.0f) * audioEnergy * 0.8f;
    ...
    void drawClusterBars(juce::Graphics& g) {
        ...
        // Centroid marker
        float centroid = proc.getSynth().getCurrentCentroid();
        float markerX = startX + centroid * 7.0f * spacing + barW * 0.5f - 1.0f;
        g.setColour(juce::Colour(0xff333333));
        g.fillRect(juce::Rectangle<float>(markerX, y0 - barMaxH - 10, 2.0f, barMaxH + 14));

        g.setColour(juce::Colour(0xffaaaaaa));
        g.setFont(juce::Font(7.0f, juce::Font::plain));
        const char* labels[] = {"SUB", "BASS", "LO-M", "MID", "HI-M", "HIGH", "AIR", "ULTRA"};
        for (int c = 0; c < 8; ++c)
            g.drawText(labels[c], (int)(startX + c * spacing - 4), (int)(y0 + 4), (int)(barW + 8), 12, juce::Justification::centred);
    }
